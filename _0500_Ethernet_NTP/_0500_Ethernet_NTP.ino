/*
 ...
 Udp NTP Client for STM32F746 Discovery
 ...
 by Arturo Guadalupi
 modified 17.Aug 2017 for STM32F746 Discovery
 by ChrsiMicro
...
char timeServer[] = "de.pool.ntp.org"; // time.nist.gov NTP server
...
void setup() 
{
  ...
  tft.println("STM32F746 Disco NTP Client");
  ...
  if (Ethernet.begin(mac) == 0) {
    ...
    tft.println("MCU stopped, to restart press reset");
...
void loop() 
{
  ...
  if (Udp.parsePacket()) 
  {
    ...
    if(tft.getCursorY()>260) // prevent cursor out of range
    {
    ...
    tft.print("UTC: ");
    int hour_=((epoch  % 86400L) / 3600);
    Serial.print(hour_); // print the hour (86400 equals secs per day)
    tft.print(hour_);
    ...
    int minute_=(epoch  % 3600) / 60;
    Serial.print(minute_); // print the minute (3600 equals secs per minute)
    tft.print(minute_);
    Serial.print(':');
    tft.print(":");
    if ((epoch % 60) < 10) {
    ...
    int second_=epoch%60;
    Serial.println(second_); // print the second
    tft.println(second_);
